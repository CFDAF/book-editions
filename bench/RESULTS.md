...
| 3 | Open: the user asked for the cons of listing every Open Library work whose title and author match | Answered after Stage 3: see Stage 3 answers, row 3 |
...
| # | Answer | Effect |
|---|---|---|
| 1 | **"Possibly the same edition" as a hint**, placed as proposed: a red margin bracket joins the two rows (a collision is always the same language and year, so they are neighbours in one language group), labelled "possibly the same edition", with a tooltip giving the reason. Opening Details on either row settles it: same ISBN → the rows fold into one with both sources; different ISBNs → the bracket goes; no ISBN on one side → the hint stays. The language count includes both rows, and a note under the list says how many pairs may be duplicates | Under answer 2 the bracket marks a pair only until both ISBNs have arrived, then only pairs where one side has none (288 such collisions in Stage 3, precision unknown). Precision where both ISBNs exist is Stage 3's 0.656 **M** |
| 2 | **One edition row listing its printing years, built in the background (option iii)**: the list shows at once; every SBN full record is fetched behind it; printings fold into editions and cross-source duplicates merge by ISBN as the ISBNs arrive; the page shows that grouping is still in progress | Measured on the Stage 3 records (`results/stage-3/q2_printings.json`) **M**: with every full record, 483 records with an ISBN become 297 edition rows (42 editions with several printings, up to 31); **295 of 778 records have no ISBN and stay one row per printing**; grouping from listing fields alone (language, publisher, title) has precision 0.425, so it is not used. It **revises decision 3** (assessment §5): SBN details are fetched during the lookup, in the background, so holdings arrive with them. **Untested**: the server has to report progress (it returns one JSON today); counts are provisional until grouping ends; background cost is one full record per SBN row (613 for N02), about 17 s for N10 at 6 at a time (**I**, from 102 s one at a time); how filters behave on partly grouped rows. **Progress carries no details unless the server streams them** (user, 2026-09-16): until the lookup has actually finished the page only says that grouping is still running, and the counts settle when it ends. Still open, and **untested**: with nothing streamed the page must still learn when grouping ended — a second request, polling, or a job id. The progress display is shown to the user before it is built (STRATEGY Step 10's rule) |
| 3 (Stage 2 q. 3) | **Main work only; the duplicate works are listed as records without their editions, and opening one fetches its editions.** The page carries a hint of this workflow. **Opening a record adds its editions to the language groups** (user, 2026-09-16), rather than opening a page of its own, so they are grouped by language and go through the same printing-grouping and duplicate check. Asked first: can a duplicate's editions be reached by looking up its title? | `results/stage-3/q3_duplicate_works.json`, the 45 duplicate works Stage 2 found (21 books):<br>- **By title, no** **M**. The Stage 1 main-work rule with the duplicate's own title, on the 33 duplicates of the 15 scored books with a main work: the main work again 24; a different book 5 (`1984` → *Animal Farm* ×2, *The Trial* → *Metamorphosis* ×3); the duplicate 3 (*Rumori*, *Cecità*, *Tra i due palazzi*, one- or two-edition Italian records); nothing 1. The planned path never searches Open Library with the typed title, so there it is 0.<br>- **By the record's key, yes** **V**: `/works/<key>/editions.json` returned every Stage 2 edition for 45 of 45; a bogus key returns 404. So "opening" a duplicate must fetch by key, not start a title lookup.<br>- **Finding the duplicates to list** **M**: the original-title search (100 results) shows 10 of 45 among author-matching results; adding each entry title raises it to 39 of 45, one extra search each (Open Library ≤ 1 request a second). Missed: *El Principito*, the French and Spanish *Se una notte…*, *Müdigkeitsgesellschaft* (every search 0 results), *Vingt poèmes…*, one *Odyssey*: titles in a language no entry title is in.<br>- **The list is not clean** (not judged): the original-title searches alone return 42 more author-matching results that pass the 0.6 title test. Among all searches they include other books and omnibus volumes (*The poems of Doctor Zhivago*, *Doctor Zhivago [adaptation]*, *The notebooks for 'Crime and punishment'*, *Animal Farm / Nineteen Eighty-Four* 37 editions, *Works (La tregua / Se questo è un uomo)* 19) and apparently more duplicates Stage 2 never saw (six one-edition *Cien años de soledad* works): Stage 2's 98 editions are a lower bound. Rows should show title and edition count so the user can tell.<br>- Requests: Open Library 145, 0 failed besides the control, 177 s, p50 0.53 s, max 7.1 s |
